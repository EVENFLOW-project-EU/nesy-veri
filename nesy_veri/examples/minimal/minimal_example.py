...
if __name__ == "__main__":
    ...
    # go through the samples to verify
    for idx in track(correctly_classified_idxs):
        input_imgs, sum_label = test_dataset[idx]

        # create perturbed input
        # maybe the first line can be outside the loop?
        ptb = PerturbationLpNorm(norm=np.inf, eps=epsilon)
        ptb_input = BoundedTensor(input_imgs, ptb)

        # perform bound propagation for each sum
        # each sum has its own network+circuit architecture
        # so we will get a lower/upper bound for each of the 19 sums
        bounds_per_sum = {
            sum_: [
                bound.item()
